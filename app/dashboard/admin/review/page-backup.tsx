...
import { getReviews, getSubmissions, updateSubmission } from "@/lib/api-client"
  ...
  useEffect(() => {
    const loadSubmissions = async () => {
      try {
        ...
        // Try to load from submissions API
        const submissionsResponse = await getSubmissions()
        if (submissionsResponse.success) {
          const allSubmissions = submissionsResponse.data || []
          console.log("📊 Total submissions from server:", allSubmissions.length)
          
          // Debug each submission
          allSubmissions.forEach((sub: any, index: number) => {
            console.log(`🔍 Submission ${index + 1}: ${sub.title || sub.judul}`)
            console.log(`   - status: ${sub.status}`)
            console.log(`   - workflow_stage: ${sub.workflow_stage}`)
            console.log(`   - content_items: ${sub.content_items?.length || 0}`)
          })
          
          // Filter submissions that need review
          const reviewSubmissions = allSubmissions.filter((sub: any) => {
            if (sub.status !== "submitted" && sub.workflow_stage !== "review") return false
            const contentItems = sub.content_items || []
            if (contentItems.length === 0) return true
            const hasPendingItems = contentItems.some((item: ContentItem) => !item.status || item.status === "pending")
            return hasPendingItems
          })
